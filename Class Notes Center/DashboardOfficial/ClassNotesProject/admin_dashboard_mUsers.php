...
function getUsers($conn){
    ...
    foreach ($users as $user) {
        ...
        echo '<button type="submit" name="change_to_admin">Make Admin</button>';
        echo '</form>';
        echo '</td>';
        echo '<td>';
        echo '<form method="POST">';
...
function deleteUser($conn, $userId) {
    try {
        ...
        // Prepare SQL statement to delete related notes
        $query1 = 'DELETE FROM Notes WHERE Users_User_Id = :user_id';
        $stmt1 = $conn->prepare($query1);
        $stmt1->bindParam(':user_id', $userId);
        $stmt1->execute();

        // Prepare SQL statements to delete user and related entries
        $query2 = 'DELETE FROM UsersCourse WHERE Users_User_Id = :user_id';
...
function changeToAdmin($conn, $userId) {
    try {
        // Prepare SQL statement to update the user type
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["change_to_admin"])) {
    ...
    // Call the changeToAdmin function
    if (changeToAdmin($conn, $userId)) {
        // Redirect to refresh the page after updating
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    } else {
        echo "Failed to change user type to admin.";
    }
}
