...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES['fileInput'])) {
    ...
    $destinationDir = "/home/altorrad/public_html/DashboardOfficial/" . $directoryName . "/";
    ...
    $noteName = $fileName; // Store the file name as the note name
    ...
    if (move_uploaded_file($fileTmpName, $filePath)) {
        ...
        try {
            ...
            $stmt->execute([$fileSize, $timestamp, $userID, $courseID, $filePath, $noteName]);
...
function getNotes($conn, $courseID){
    ...
    foreach ($notes as $note) {
        ...
        echo '<td>' . htmlspecialchars($note['Note_Name']) . '</td>'; // Display the Note_Name
        echo '<td><a href="' . htmlspecialchars($note['Note_Path']) . '" download>Download</a></td>';
        echo '</tr>';
    }
}
?>
...
<body>
    ...
    <script>
        ...
        function uploadNote() {
            // You can add JavaScript code here to handle the upload process if needed
            // This function can be extended to perform AJAX requests to handle file upload
        }
    </script>
</body>
</html>
